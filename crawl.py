...
class MyHTMLParser(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag == 'script' and attr['src']:
            ...
            if (not _RE_SCRIPT_LINK.match(attr['href'])) and (not _RE_VOID_LINK.match(attr['href'])):
    ...
    def handle_startendtag(self, tag, attrs):
        ...
        if tag == 'img' and attr['src']:
            if (not _RE_SCRIPT_LINK.match(attr['src'])) and (not _RE_VOID_LINK.match(attr['src'])):
                url_list.put(attr['src'])
                log('Put %s into queue' % attr['src'])
